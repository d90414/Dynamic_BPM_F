...
angular.module('ASS.account').controller('ShengfenCtrl', ['$rootScope', '$scope', '$window', '$state', 'accountService', 'myAlert','$http','bpmRoot','bpmService',
    function ($rootScope, $scope, $window, $state, accountService, myAlert,$http,bpmRoot,bpmService) {
      if(!$rootScope.flowName){
        ...
        $scope.coderesult = 1;
        $scope.userId = "50038219870416";
         $scope.flowTaskName = "身份验证";
      ...
      $scope.shengfengcheck = function(){
            if ( "500382198704161234"== $scope.userId){
                $scope.coderesult = 2;
            }
        }
